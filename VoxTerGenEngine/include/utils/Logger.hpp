 ...
 *
 * Provides logging to both console and file with timestamps (UTC and local),
 * supports different log levels, and uses C++20 std::format for formatting.
 */
class Logger
{
private:
	static inline std::ofstream log_file_;   /**< File stream for logging */
	static inline std::mutex log_mutex_;     /**< Mutex for thread safety */

	static inline bool console_enabled_ = false; /**< Enable console output */
	static inline bool file_enabled_ = false;    /**< Enable file output */
	static inline bool initialized_ = false;     /**< Tracks if the logger has been initialized */
...
public:
	 ...
	 * @param console_enabled Whether console logging is enabled
	 * @param file_enabled Whether file logging is enabled
	 */
